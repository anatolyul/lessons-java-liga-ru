...
public class Truck {
     ...
     */
    public Truck(String truckName, List<BoxDto> boxes) {
     ...
     */
    private void addBox(BoxDto box) {
        List<BoxDto> boxList = this.getBoxes() != null ? this.getBoxes() : new ArrayList<>();
        boxList.add(box);
     ...
     */
    public boolean canLoadBox(BoxDto box) {
     ...
     */
    public void loadBox(BoxDto box) {
        for (int i = 0; i <= truckHeight - box.getHeight(); i++) {
            for (int j = 0; j <= truckWidth - box.getWidth(); j++) {
                if (canPlaceBox(box, i, j)) {
                    placeBox(box, i, j);
     ...
     */
    private boolean canPlaceBox(BoxDto box, int startHeight, int startWidth) {
     ...
     * @param startWidth  начальная ширина для размещения
     */
    public void placeBox(BoxDto box, int startHeight, int startWidth) {
        for (int i = 0; i < box.getHeight(); i++) {
            for (int j = 0; j < box.getWidth(); j++) {
